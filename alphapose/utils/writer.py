    ...
    def update(self):
        ...
        while True:
            ...
            if boxes is None:
                ...
                if self.opt.jsonfile and (self.opt.save_img or self.save_video or self.opt.vis):
                    from alphapose.utils.vis import leer_json
                    img = leer_json(orig_img, self.opt.jsonfile, self.opt.people, add_bbox=(self.opt.pose_track | self.opt.tracking))
                    self.write_image(img, im_name, stream=stream if self.save_video else None)
                elif self.opt.save_img or self.save_video or self.opt.vis:
                    if hm_data.size()[1] == 49:
                    ...
                    img = vis_frame(orig_img, result, self.opt.people, add_bbox=(self.opt.pose_track | self.opt.tracking))
                    self.write_image(img, im_name, stream=stream if self.save_video else None)

    def startJSON(self):
        # start a thread to write pose estimation results per frame
        self.result_worker = self.start_worker(self.updateJSON)
        return self
