...
from alphapose.coco.coco import COCO
from alphapose.coco.cocoeval import COCOeval
...
def evaluate_mAP(res_file, ann_type='bbox', ann_file='person_keypoints_val2017.json', silence=True):
    ...
    ann_file = os.path.join('/pub/databases/coco/annotations/', ann_file)
    ...
    #cocoEval.params.kpt_oks_sigmas = np.zeros(6)
    #print(cocoEval.params.kpt_oks_sigmas)
    #print("SIZE")
    #print(cocoEval.params.kpt_oks_sigmas.size)

    #if ann_type == "keypoints":
        #num_keypoints = 6
        #assert cocoEval.params.kpt_oks_sigmas.size == num_keypoints
...
def calc_accuracy(preds, labels):
    ...
    #preds.shape[0] -> BATCH_SIZE
    #preds.shape[1] -> JOINTS
    #preds.shape[2] -> HEATMAP HEIGHT
    #preds.shape[3] -> HEATMAP WEIGHT
...
def calc_dist(preds, target, normalize):
    ...
    for n in range(preds.shape[0]):
        for c in range(preds.shape[1]):
            if target[n, c, 0] > 1 and target[n, c, 1] > 1:
                #print("BATCH: ", n, ", JOINT:", c)
                #print("PRED: ", preds[n, c, :])
                #print("TARGET: ", target[n, c, :])
                normed_preds = preds[n, c, :] / normalize[n]
                normed_targets = target[n, c, :] / normalize[n]
                dists[c, n] = np.linalg.norm(normed_preds - normed_targets)
            else:
